...
+ The data from that dataset comes from real estate listings posted on [Alo Nhà Đất](https://alonhadat.com.vn) that was
  by [Le Anh Duc](https://www.kaggle.com/ladcva)
...
1. Download the zipped project folder. Unzip it by double-clicking on it.However, you can clone this project through link Github. 
...
4. Click on housePrediction.ipynb in the browser tab. This will open up my main file in the Jupyter Lab.
...
4. In case you want to have a new dataset, you need to input it into the housePrediction.ipynb and run all the cells
...
4. In case you want to have a new dataset, you need to input it inot the housePrediction.ipynb and run all the cells
...
│     └── app.py
├── check_features.py   <- The features of the model
├── debug_model.py      <- Debug the model
├──error.txt           <- The errors of the model
├── feature_list.txt        <- The features of the model
├──get_features.py       <- The features of the model
├──inspect_pipeline.py   <- The features of the model
|
|── report.pdf 
|
|
